...
exports.map_update_get = asyncHandler(async (req, res, next) => {
  const [map, allCartographers, allMaptypes] = await Promise.all([
    Map.findById(req.params.id).populate("cartographer").exec(),
    Cartographer.find().sort({ family_name: 1 }).exec(),
    MapType.find().sort({ name: 1 }).exec(),
  ]);
  ...
  // Mark our selected MapTypes as checked.
  allMaptypes.forEach((maptype) => {
    if (map.maptype.includes(maptype._id)) maptype.checked = "true";
  });
...
exports.map_update_post = [
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      const updatedMap = await Map.findByIdAndUpdate(req.params.id, map, {});
